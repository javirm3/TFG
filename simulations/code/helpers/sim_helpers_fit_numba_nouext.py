...
@njit
def simulate_path_order2_numba(side_code, IL, IC, IR, sL, sC, sR,
                               Tmax, dt=0.1/40, noise_amp=0.5):
    x0 = np.array([0.0, 0.0])
    N = int(Tmax / dt)
    X = np.empty((N + 1, 2))
    X[0] = x0
    ...
    for i in range(N):
        ...
        dB1 = (dW[0] - dW[1]) / 2
        dB2 = (dW[0] + dW[1] - 2*dW[2]) / 6
        noise = noise_amp * np.array([dB1, dB2])

        # 2) Drift en el punto actual
        d1 = drift_numba(X[i], IL[i], IC[i], IR[i], sL, sC, sR)

        # 3) Predictor
        X_pred = X[i] + d1 * dt + noise

        # 4) Drift en el predictor
        d2 = drift_numba(X_pred, IL[i], IC[i], IR[i], sL, sC, sR)

        # 5) Corrector: promedio de drifts + mismo ruido
        X[i+1] = X[i] + 0.5 * (d1 + d2) * dt + noise

    # … el resto igual que antes para decidir la salida
    r1 = X[-1, 0] + X[-1, 1]
    r2 = -X[-1, 0] + X[-1, 1]
    r3 = -2.0 * X[-1, 1]
...
def run_trial_numba_nouext(side, S_params, U_int_params, s_params, Tmax, dt=0.1/40, noise_amp=0.5):
    t_vec = np.linspace(0, Tmax, int(Tmax / dt) + 1)
