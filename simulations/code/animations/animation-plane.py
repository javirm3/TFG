...
I_fast1 = np.linspace(-2, -0.5, 30)     # rápido (tramo inicial)
I_add = [0]
I_slow  = np.linspace(-0.5, 0.5, 90)    # lento (más frames en zona central)
I_fast2 = np.linspace(0.5, 2, 30)       # rápido (tramo final)

I_vals = np.concatenate([I_fast1, I_add, I_slow, I_fast2])
...
def update(frame):
    ...
    "BlueWhiteTeal",
    ["#2A398D", "#FFFFFF", "#007A82"]
    )

    contour = ax.contour(X, Y, Z, 15, levels=30, cmap=cmap, alpha=1)

    ax.contour(X, Y, Z, 15, levels=30, cmap=cmap, alpha=1)
    # ax.contour(Xg, Yg, Vg, levels=30, cmap=cmap, alpha=1)
    ...
    colors = {'Maximum': 'orange', 'Minimum': 'blue', 'Saddle Point': 'purple'}
    ...
    ax.set_title(f'Potential Landscape $\\psi(X_1, X_2)$ at $I_L=I_R=I_C={I_val:.2f}$', fontsize=16)
...
current_subs_dict = subs_dict.copy()
current_subs_dict.update({
    sp.symbols('I_L'): I_val_init,
    sp.symbols('I_C'): I_val_init,
    sp.symbols('I_R'): I_val_init
})

pot_expr_num = sp.N(U_simpl.subs(current_subs_dict))
potencial_num = sp.lambdify((sp.symbols('X1'), sp.symbols('X2')), pot_expr_num, 'numpy')
Z_init = potencial_num(X, Y)
Z_init = np.clip(Z_init, -50, 50)

# Crear el contour inicial para el colorbar
cmap = LinearSegmentedColormap.from_list("BlueWhiteTeal", ["#2A398D", "#FFFFFF", "#007A82"])
contour_plot = ax.contourf(X, Y, Z_init, levels=30, cmap=cmap)

# Añadir colorbar
cbar = fig.colorbar(contour_plot, ax=ax)
# Crear la animación
anim = FuncAnimation(fig, update, frames=len(I_vals), interval=100, blit=True)

# Guardar la animación
anim.save('potential_landscape_animation.gif', fps=10)
plt.close()