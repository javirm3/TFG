...
from sim_helpers_fit_numba import drift_numba as global_drift
...
df_all = pd.read_csv(f'{storage_path}/df_filtered.csv', sep=';')
...
def objective_function(params):
    try:
        ...
        with ProcessPoolExecutor(max_workers=10, initializer=init_worker, initargs=(params, drift)) as executor:
        ...
        # notify_telegram(f"❌ Error durante la simulación: {e}")
        return 1.0  # Penaliza si algo falla


if __name__ == '__main__':
    # Lista de sujetos
    subjects = ['A92', 'A86', 'A87','A88', 'A89']  # Añade aquí los sujetos que quieras procesar

    for subject in subjects:
        print(f"\n🚀 Empezando optimización para subject {subject}")
        # notify_telegram(f"🚀 Empezando optimización para subject {subject}")
        df = df_all[(df_all['subject'] == subject) & (df_all['timepoint_4'] <= 5)].copy()
        
        if df.empty:
            print(f"⚠️ No hay datos para subject {subject}, saltando...")
            continue

        rows = [(row.stimd_c, row.ttype_c, row.timepoint_1, row.timepoint_2, row.timepoint_3,
                 row.timepoint_4, row.x_c, row.r_c, row.ttype_c) for row in df.itertuples()]

        # def objective_function(params):
        #     try:
        #         with ProcessPoolExecutor(max_workers=48, initializer=init_worker, initargs=(params, drift)) as executor:
        #             errors = list(executor.map(simulate_row_pure, rows))
        #         sse = sum(errors)
        #         obj = 1 - (sse / len(rows))
        #         if not np.isfinite(obj): return 1.0
        #         if obj == 0 or obj == 1.0:
        #             obj = obj - 1e-6 if obj == 1.0 else obj + 1e-6
        #         return obj
        #     except Exception as e:
        #         print(f"❌ Error en {subject}: {e}")
        #         notify_telegram(f"❌ Error durantae {subject}: {e}")
        #         return 1.0
        ...
        bads = BADS(
            ...
            options={"uncertainty_handling": True, "max_fun_evals": 5000}
        )
        ...
        print("Valor óptimo:", result.fval)
