...
Furthermore, by including synaptic plasticity in the rate circuit, the model can account for the adaptation of the biases to the choice history statistics, just as observed in mice during a variant of the task with non-uniform stimulus statistics

Simulations of the reduced system can mimic mice choice behavior

Our Neural circuit rate model successfully replicates animals’ behavior in the 3CDR task providing a mechanistic explanation and serving as a quantitative tool to interpret neural data and manipulation experiments.


Next steps:
- Idiosyncratic biases with a dynamic update of alpha and how they shape the potential
- Probability of escapement of well
- Fitting to the behavioral data

\section{Discussion and Conclusions}
In this study we have shown that a simple three‐population firing‐rate network, endowed with a non‐selective urgency ramp and transient sensory inputs, can quantitatively reproduce the key behavioral signatures of mice performing the 3CDR task. As illustrated in Figure~\ref{fig:sim-trial-types}, the model captures the decline in choice accuracy from Easy through Hard trials, and—when applied to the Variable–Stimulus and Variable–Delay protocols—it likewise predicts performance drops for both very brief and very late cues (Figures~\ref{fig:sim-stim-dur} and \ref{fig:sim-delay-dur}). These findings suggest that errors arise primarily from the temporal misalignment between evidence availability and the rising urgency signal, rather than from an intrinsic decay of memory during the delay.
...
Our model generalizes two‐choice urgency‐signal frameworks \cite{inagaki2019,wong2006} to multiple alternatives in a principled way. Unlike classical DDMs that assume infinite memory, our formulation shows that even short delays can produce near‐chance performance if they misalign with an urgency signal, shifting the interpretation of “memory limitations” in delayed‐response tasks. Nonetheless, we have made simplifying assumptions: urgency is implemented as a fixed linear ramp and we have not yet fitted or modeled individual mice’s idiosyncratic biases or their trial‐by‐trial adaptation.
